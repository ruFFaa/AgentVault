...
from fastapi import status, HTTPException, Depends, Body, Query, Response # Added Response, BackgroundTasks
...
# TODO: Re-enable this test when registration is enabled.
...
#
#     # --- Assert ---
#     assert response.status_code == status.HTTP_201_CREATED # <<< THIS WILL FAIL NOW (expects 503)
...
#
#     # --- Assert ---
#     assert response.status_code == status.HTTP_400_BAD_REQUEST # <<< THIS WILL FAIL NOW (expects 503)
...
#
#     # --- Assert ---
#     assert response.status_code == status.HTTP_409_CONFLICT # <<< THIS WILL FAIL NOW (expects 503)
...
# TODO: Re-enable this test (or adjust assertion) when registration/verification is enabled and login requires verification again.
# @patch("agentvault_registry.routers.auth.developer_crud.get_developer_by_email", new_callable=AsyncMock)
...
#
#     # --- Assert ---
#     assert response.status_code == status.HTTP_401_UNAUTHORIZED # <<< THIS WILL FAIL NOW (expects 200)
