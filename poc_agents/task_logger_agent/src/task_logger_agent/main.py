...
# Import agent logic
from .agent import TaskLoggerAgent
...
# --- FastAPI App Setup ---
app = FastAPI(
    title="Task Logger Agent",
    description="An agent that logs messages to a PostgreSQL database with timestamps.",
    version="0.1.0"
)
...
agent_instance = TaskLoggerAgent(task_store_ref=task_store)
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Task Logger Agent running"}
...
logger.info("Task Logger Agent application initialized.")
