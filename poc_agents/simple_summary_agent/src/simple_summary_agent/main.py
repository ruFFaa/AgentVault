...
# Import agent logic
from .agent import SimpleSummaryAgent
...
# --- FastAPI App Setup ---
app = FastAPI(
    title="Simple Summary Agent",
    description="An agent that generates summaries of text using a local LLM (LM Studio).",
    version="0.1.0"
)
...
agent_instance = SimpleSummaryAgent(task_store_ref=task_store)
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Simple Summary Agent running"}
...
logger.info("Simple Summary Agent application initialized.")
